...
public class Stokvel {
    ...
    @Column(nullable = false)
    @Builder.Default
    private StokvelStatus status = StokvelStatus.FORMING;
    ...
    @Column
    @Builder.Default
    private Integer currentPayoutIndex = 0;
    @OneToMany(mappedBy = "stokvel", cascade = CascadeType.ALL, orphanRemoval = true)
    @Builder.Default
    private List<StokvelMember> members = new ArrayList<>();
    @OneToMany(mappedBy = "stokvel", cascade = CascadeType.ALL, orphanRemoval = true)
    @Builder.Default
