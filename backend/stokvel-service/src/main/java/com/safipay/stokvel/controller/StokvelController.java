...
public class StokvelController {
    ...
    @GetMapping
    public ResponseEntity<ApiResponse<List<StokvelResponse>>> getAll() {
        return ResponseEntity.ok(ApiResponse.success(svc.getAll()));
    }

    @GetMapping("/{id}")
    public ResponseEntity<ApiResponse<StokvelResponse>> getById(@PathVariable Long id) {
        return ResponseEntity.ok(ApiResponse.success(svc.getById(id)));
    }

    @GetMapping("/my")
    public ResponseEntity<ApiResponse<List<StokvelResponse>>> getMine(@AuthenticationPrincipal String userId) {
        return ResponseEntity.ok(ApiResponse.success(svc.getMine(userId)));
    }

    @GetMapping("/{id}/contributions")
    public ResponseEntity<ApiResponse<List<ContributionResponse>>> getContributions(@PathVariable Long id, @AuthenticationPrincipal String userId) {
        return ResponseEntity.ok(ApiResponse.success(svc.getContributions(id, userId)));
    }

    @GetMapping("/{id}/payouts")
    public ResponseEntity<ApiResponse<List<PayoutResponse>>> getPayouts(@PathVariable Long id, @AuthenticationPrincipal String userId) {
        return ResponseEntity.ok(ApiResponse.success(svc.getPayouts(id, userId)));
    }
}
